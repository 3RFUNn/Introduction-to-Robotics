...
n = 5
rows_up = [0, 62, 125, 20, 80, 72]
rows_down = [0, 45, 0, 90, 30, 36]
columns_up = [0, 150, 160, 235, 145, 105]
columns_down = [0, 120, 110, 35, 125, 165]


def predict_one_box(i):
    im_gray = cv2.imread(
        f"../../datasets/box{i}/output_image{i}-1.jpg", cv2.IMREAD_GRAYSCALE
    )
    ...
    cv2.imwrite(
        f"../../datasets/box{i}/output_image{i}-1_gray_scale_crop.jpg", im_gray_crop
    )

    im_gray_crop_resize = resize_image(im_gray_crop, (28, 28))
    cv2.imwrite(
        f"../../datasets/box{i}/output_image{i}-1_gray_scale_crop_resize.jpg",
        im_gray_crop_resize,
    )
        ...
        if all([abs(x1 - x2) < self.target_precision for (x1, x2) in zip(self.target_position, self.current_pose[0:2])]):
            ...
            if verbose_target:
                while self.step(self.time_step) != -1:
                    ...
                    if abs(yaw) > 3.13:
                        # t1 = self.getTime()
                        # while self.getTime() - t1 < 0.1:
                        #     print('waiting')
                        
                        #### ARMAN INJA CODETO BEZAR!!!
                        #### Basheeee!!!
                        n = 1
